...
for n = 1:48
    ...
    [AllTrials2,eraseday, isgood] = NormalizeActiv(AllTrials, n, days,hitNorm ,bf, pf2);
    %      AllTrials = NormalizeMUA(AllTrials, n, days,hitNorm ,bf, pf);
    ...
    for ar = 1:2
        for n = (24*(ar-1)+1):(24*ar)
            ...
            if plot_stim_dur
                ...
                pause(0.02)
                set(gcf,'PaperPositionMode','auto')
                print(gcf,'-dpng',[SaveDir 'ConnUnconnNoDur_s' num2str(days(1)) '_' num2str(days(end)) '_array_' num2str(ar) '.png'])
...
for ar = 1:2
    good_chann = find(SNR>0.6);
    if ar==1
        good_chann(good_chann>24) = [];
    else
        good_chann(good_chann<25) = [];
    end
    if plot_chann
        ...
        filename = ['CorrOffsetWorth_' num2str(Sessions(1)) '_' num2str(Sessions(end)) '_array_' num2str(ar) ];
        set(gcf, 'PaperPositionMode', 'auto')
    ...
    if plot_stim_dur
        ...
        figure(f1)
        filename = ['NeurActiv_withStimDur_noMask_array_' num2str(ar)'];
        set(gcf, 'PaperPositionMode', 'auto')
        print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
        if plot_mask
            figure(f2)
            filename = ['NeurActiv_withStimDur_Mask_' num2str(Sessions(1)) '_' num2str(Sessions(end)) '_array_' num2str(ar)];
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
        end
        %%
        ...
        filename = ['Diff_ConnectedUnconn_' num2str(Sessions(1)) '_' num2str(Sessions(end)) '_array_' num2str(ar)];
        set(gcf, 'PaperPositionMode', 'auto')
        print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
        ...
        [legend_h,object_h,plot_h,text_strings] = legend('no mask', 'masked', 'Location', 'NorthWest');
        LegendPos = get(legend_h,'Position');
        EstimWorthNeuronal_avg = (beta(1)-beta(3))/beta(2);
        dim = [LegendPos(1) LegendPos(2)-LegendPos(4) .3 LegendPos(4)/2];
        str = ['Estimated worth: ' num2str(EstimWorthNeuronal_avg,3) 'ms'];
        annotation('textbox',dim,'String',str,'FitBoxToText','on', 'LineStyle', 'none')
        
        title('Information about the connections in function of stimulus duration')
        xlabel('stimulus duration')
        
        filename = ['Info_stimDur' num2str(Sessions(1)) '_' num2str(Sessions(end)) '_array_' num2str(ar)];
        set(gcf, 'PaperPositionMode', 'auto')
        print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
        %%
        ...
        if 0
            ...
            for sd = 1:min(numel(Poss_dur),7)
                ...
                AUC(sd) = sum(smooth(t_connected_avg{sd}(t_relev)-t_unconnected_avg{sd}(t_relev),20));
            end
            ...
            if plot_mask
                ...
                for sd = 1:min(numel(Poss_dur),7)
                    t_relev = find(tb>(Poss_dur(sd)/1e3) & tb<(Poss_dur(sd)/1e3 +.3));
                    AUC_m(sd) = sum(smooth(t_connected_m_avg{sd}(t_relev)-t_unconnected_m_avg{sd}(t_relev), 20));
                end
            ...
            filename = ['Diff_ConnectedUnconn_AUC_array_' num2str(ar)];
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
