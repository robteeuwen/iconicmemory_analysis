...
if isempty(Stm_INF), disp('Error no stimulus events')
else
    ...
    if ~isempty(Stm_INF)
        ...
        Trilist(:,1) = Stm_INF; %stimulus onsets(1)
    end
    ...
    for i = 1:TrlNm
        ...
        if ~isempty(Sacc_INF)
            %saccade onsets (3)
            if i < TrlNm
                Ixm = find(Sacc_INF > Stm_INF(i) & Sacc_INF < Stm_INF(i+1));
            else
                Ixm = find(Sacc_INF > Stm_INF(i), 1, 'first');
            end
            if ~isempty(Ixm)
        ...
        if ~isempty(Targ_INF)
            %target onsets (4)
            if i < TrlNm
                Ixm = find(Targ_INF > Stm_INF(i) & Targ_INF < Stm_INF(i+1));
            else
                Ixm = find(Targ_INF > Stm_INF(i), 1, 'first');
            end
            if ~isempty(Ixm)
        ...
        if ~isempty(Corr_INF)
            %corrects (5)
            if i < TrlNm
                Ixk = find(Corr_INF > Stm_INF(i) & Corr_INF < Stm_INF(i+1));
            else
        ...
        if ~isempty(Rewd_INF)
            %rewards (6)
            if i < TrlNm
                Ixk = find(Rewd_INF > Stm_INF(i) & Rewd_INF < Stm_INF(i+1));
            else
                Ixk = find(Rewd_INF > Stm_INF(i));
            end
            if ~isempty(Ixk)
        ...
        if ~isempty(Err_INF)
            %errors (7)
            if i < TrlNm
                Ixk = find(Err_INF > Stm_INF(i) & Err_INF < Stm_INF(i+1));
            else
                Ixk = find(Err_INF > Stm_INF(i), 1, 'first');
            end
            if ~isempty(Ixk)
        ...
        if ~isempty(Word_INF)
          ...
          if ~isempty(Trl_INF)
            %words (9)
            if i == 1
                Ixk = find(Trl_INF(:,1) < Stm_INF(i), 1, 'last');
            else
                Ixk = find(Trl_INF(:,1) > Stm_INF(i-1) & Trl_INF(:,1) < Stm_INF(i), 1, 'last');
            end
            if ~isempty(Ixk)
                Trilist(i,2) = Trl_INF(Ixk(1),1);  %conditional information
            else Trilist(i,2) = nan;
            end
        end
