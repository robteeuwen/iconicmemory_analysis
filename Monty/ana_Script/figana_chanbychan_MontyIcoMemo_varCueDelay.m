% analysis function for the initial cueing part of the iconic memory
% project
% sessions 72 - 93
...
%91-93 biaised version of const segm
Sessions = 93;%[81:84 86];%72:75;%72:74;%
        ...
        for n = ana_chan %48
            ...
            for h = 1:2
                if plot_stim_dur
                    ...
                    ind = find(isdrawn4==0 & Hit==h  & mask ==1 & stim_dur==Poss_dur(stimcond));%& stim_dur == Poss_Cue_Time(sd) & mask == m
                    background{n,h}  = nanmean(AllTrials(:,ind),2);
                    if ~isempty(find(condcheck(ind))); keyboard;end
                    condcheck(ind) = 1;
                    
                    ind = find(targ==10 & Hit==h  & mask ==1 & stim_dur==Poss_dur(stimcond));
                    ttree_t{n,h} = nanmean(AllTrials(:,ind),2);
                    if ~isempty(find(condcheck(ind))); keyboard;end
                    condcheck(ind) = 2;
                    
                    
                    ind = find(targ==9 & Hit==h  & mask ==1 & stim_dur==Poss_dur(stimcond));
                    ttree_d{n,h} = nanmean(AllTrials(:,ind),2);
                    if ~isempty(find(condcheck(ind))); keyboard;end
                    condcheck(ind) = 3;
                    
                    ind = find(connected==1 & targ~=10 & Hit==h & mask ==1 & stim_dur==Poss_dur(stimcond));
                    dtree_t{n,h} = nanmean(AllTrials(:,ind),2);
                    if ~isempty(find(condcheck(ind))); keyboard;end
                    condcheck(ind) = 4;
                    
                    ind = find(connected==2 & targ~=9 & Hit==h & mask ==1 & stim_dur==Poss_dur(stimcond));
                    dtree_d{n,h} = nanmean(AllTrials(:,ind),2);
                    if ~isempty(find(condcheck(ind))); keyboard;end
                    condcheck(ind) = 5;
                    
                    ind = find( Hit==h  & mask ==1 & stim_dur==Poss_dur(stimcond) );
                    perf(h) = numel(ind);
                    
                    
                    ind1 = find(connected==1  & mask ==1 & Hit==2 & stim_dur==Poss_dur(stimcond));%
                    t_connected{n} =  nanmean(AllTrials(:,ind1),2);
                    ind2 = find(connected==2  & mask ==1 & Hit==2 & stim_dur==Poss_dur(stimcond));%
                    ...
                    if plot_mask
                        for sd = 1:numel(Poss_Cue_Time)
                            ind = find(isdrawn4==0 & Hit==h & cue_time == Poss_Cue_Time(sd) & mask ==3 & stim_dur==Poss_dur(stimcond));%& stim_dur == Poss_Cue_Time(sd) & mask == m
                            background_m{n,h,sd}  = nanmean(AllTrials(:,ind),2);
                            if ~isempty(find(condcheck(ind))); keyboard;end
                            condcheck(ind) = 11;
                            
                            ind = find(targ==10 & Hit==h & cue_time == Poss_Cue_Time(sd) & mask ==3 & stim_dur==Poss_dur(stimcond));
                            ttree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                            if ~isempty(find(condcheck(ind))); keyboard;end
                            condcheck(ind) = 12;
                            
                            
                            ind = find(targ==9 & Hit==h & cue_time == Poss_Cue_Time(sd) & mask ==3 & stim_dur==Poss_dur(stimcond));
                            ttree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                            if ~isempty(find(condcheck(ind))); keyboard;end
                            condcheck(ind) = 13;
                            
                            ind = find(connected==1 & targ~=10 & Hit==h & cue_time == Poss_Cue_Time(sd)& mask ==3  & stim_dur==Poss_dur(stimcond));
                            dtree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                            if ~isempty(find(condcheck(ind))); keyboard;end
                            condcheck(ind) = 14;
                            
                            ind = find(connected==2 & targ~=9 & Hit==h & cue_time == Poss_Cue_Time(sd)& mask ==3 & stim_dur==Poss_dur(stimcond) );
                            dtree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                            if ~isempty(find(condcheck(ind))); keyboard;end
                            condcheck(ind) = 15;
                            
                            ind = find( Hit==h & cue_time == Poss_Cue_Time(sd) & mask ==3 & stim_dur==Poss_dur(stimcond));
                            perf_m(h,sd) = numel(ind);
                            
                            
                            ind = find(connected==1 & cue_time == Poss_Cue_Time(sd) & mask ==3 & Hit==2 & stim_dur==Poss_dur(stimcond));%
                            t_connected_m{n,sd} =  nanmean(AllTrials(:,ind),2);
                            ind = find(connected==2 & cue_time == Poss_Cue_Time(sd) & mask ==3 & Hit==2 & stim_dur==Poss_dur(stimcond));%
...
%                     
%                     if plot_mask
%                         ind = find(isdrawn4==0 & Hit==h & mask ==3  & stim_dur==Poss_dur(stimcond));%& stim_dur == Poss_Cue_Time(sd) & mask == m
...
%                     end
        ...
        if plot_stim_dur
            ...
            %%
            f1 = figure('Position', [100 50 1400 900]);
            if plot_mask; f2 = figure('Position', [100 50 1400 900]);f3 = figure('Position', [100 50 1400 900]);end
            for sd = 1:min(numel(Poss_Cue_Time),6)
                ...
                plot(tb,smooth(dtree_d_avg{2},smoothness),'m', 'LineWidth',2); xlim([-.4 0.9]);
                if plot_mask
                    figure(f2);subplot(2,2,sd);hold on; title([num2str(Poss_Cue_Time(sd)), ' ms, cued at 20ms'])%
                    ...
                    plot(tb,smooth(dtree_d_m_avg{2,sd},smoothness),'m', 'LineWidth',2); xlim([-.4 0.9]);
                    
                    
                end
            end
            figure(f1)
            filename = ['NeurActiv_withStimDur_noCue_' StimCondStr{stimcond} 'Stim_s' num2str(Sessions(1)) '_' num2str(Sessions(end)) '_chan' num2str(ana_chan(1)) '_' num2str(ana_chan(end))];
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
            if plot_mask
                figure(f2)
            ...
            if plot_mask
                ...
                xlim([-.2 .9]);
                legend(['Cue at ' num2str(Poss_Cue_Time(1)) 'ms'], ['Cue at ' num2str(Poss_Cue_Time(2)) 'ms'],['Cue at ' num2str(Poss_Cue_Time(3)) 'ms'],['Cue at ' num2str(Poss_Cue_Time(4)) 'ms'])
            ...
            for sd = 1:min(numel(Poss_Cue_Time),6)
                ...
                if plot_mask
                    ...
                    ttree_t_m_avg{2,sd} = ttree_t_m_avg{2,sd}-mean(ttree_t_m_avg{2,sd}(tb<baseline(2)));
                    %             t_unconnected_m_avg{sd} = t_unconnected_m_avg{sd}-mean(t_unconnected_m_avg{sd}(tb<0));
                    dtree_t_m_avg{2,sd} = dtree_t_m_avg{2,sd}-mean(dtree_t_m_avg{2,sd}(tb<baseline(2)));
            ...
            for sd = 1:min(numel(Poss_Cue_Time),6)
                ...
                if plot_mask
                    %             t_connected_m_avg{sd} = t_connected_m_avg{sd}-mean(t_connected_m_avg{sd}(tb<0));
                    ttree_t_m_avg{2,sd} = ttree_t_m_avg{2,sd}-mean(ttree_t_m_avg{2,sd}(tb<baseline(2)));
                    ttree_d_m_avg{2,sd} = ttree_d_m_avg{2,sd}-mean(ttree_d_m_avg{2,sd}(tb<baseline(2)));
                    %             t_unconnected_m_avg{sd} = t_unconnected_m_avg{sd}-mean(t_unconnected_m_avg{sd}(tb<0));
                    dtree_t_m_avg{2,sd} = dtree_t_m_avg{2,sd}-mean(dtree_t_m_avg{2,sd}(tb<baseline(2)));
                    dtree_d_m_avg{2,sd} = dtree_d_m_avg{2,sd}-mean(dtree_d_m_avg{2,sd}(tb<baseline(2)));
