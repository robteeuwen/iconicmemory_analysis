...
%                         keyboard
            ...
            corrected_cue_time = [-unique(LOG.Precue_dur) Poss_Cue_Time(2:end)+fix_stim_dur*1000]./1000;
            ...
            %filter screen refresh
            if 0
                AllTrials3 = nan(size(AllTrials2,1),size(AllTrials2,2));
                fs = EVENT.strms(1).sampf;             %#sampling rate
                f0 = 60;                %#notch frequency
                fn = fs/2;              %#Nyquist frequency
                freqRatio = f0/fn;      %#ratio of notch freq. to Nyquist freq.

                notchWidth = 0.2;       %#width of the notch
                ...
                for tt = 1:size(AllTrials2,2)

                    %#filter signal x
                    AllTrials3(:,tt) = filter(b,a,AllTrials2(:,tt));
                    
                    %AllTrials3(:,tt) = bandpass(AllTrials2(:,tt),[84 86], EVENT.strms(1).sampf);
                end
                AllTrials2 = AllTrials3; 
            end
        ...
        %% NEW VERSION OF plot the average over an array for all the conditions
        f2 = figure('Position', [394         234        1087         578]);
        for ct = 1:min(numel(Poss_Cue_Time),6)
            % compute the average
            for h = 1:2
        ...
        for ct = 1:min(numel(Poss_Cue_Time),6)
            ...
            integrationstart = corrected_cue_time(ct); 
            integrationend = 750/1000;
            integrationdur = integrationend-integrationstart;
            
            topplot = ct+4*floor((ct-1)/4);
            bottomplot = ct+4*floor((ct-1)/4)+4;

            % top plot
            subplot(4,4,topplot);hold on; title(['cue ISI:' num2str(Poss_Cue_Time(ct)), ' ms']);
            if ct==1
                title('precue');
            end
            ylim1 = [-0.2 0.8];
            ...
            for i=1:2
                ...
                % cue onset
                line([corrected_cue_time(ct) corrected_cue_time(ct)], [-10 20],'Color',[0 1 0],'LineWidth',1)

                % response onset
                l2 = line([650/1000 650/1000],[-10 20],'Color',[0 0 0],'LineWidth',1);
                
                % integration window
                p=patch([integrationstart integrationend integrationend integrationstart],[-10 -10 20 20],'r');
                t_relev = find(tb>(integrationstart) & tb<(integrationend));
...
%         plot(tb, smooth(ttree_t_avg{2}-dtree_t_avg{2}, smoothness), 'k', 'LineWidth',2); 
        ...
        input('press any key for next array')
