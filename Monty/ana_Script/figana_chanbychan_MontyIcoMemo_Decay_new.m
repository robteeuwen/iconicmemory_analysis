...
% Monty - sessions 20180315 and later.
% function created by Catherine Wacongne (catherine.waco@gmail.com)
% final: sessions 141-149 (20180413 - 20180424)
%
close all
clearvars
dbstop if error
...
%         xlim([-.2 .9]);
        ...
        %% optimized figure for differences between connected and unconnected for various mask onsets
...
%         AUC_avg = zeros(1,min(numel(Poss_CueTime),6));
        ...
        for cd = 1:min(numel(Poss_CueTime),7)
            subplot(2,4,cd);hold on; title(['Stim duration: ' num2str(Poss_CueTime(cd)), ' ms']); xlabel('time(s)'); ylabel('normalized activity')
            ylim1 = [-2 2];
            if 1
                a = plot(tb, smooth(connected_m_avg{2,cd}, smoothness), 'b', 'LineWidth',1); ylim(ylim1)
                [hAx,hLine1,hLine2] = plotyy(tb, smooth(unconnected_m_avg{2,cd}, smoothness), tb, smooth(connected_m_avg{2,cd}-unconnected_m_avg{2,cd}, smoothness));xlim([-.2 0.9]);
                hAx(2).XLim = [-.2 0.9];
                hLine1.LineWidth = 1;
                hLine1.Color = [1 0 0];
                hAx(1).YLim = ylim1;
