...
% Monty - sessions 20180305 and later.
% function created by Catherine Wacongne
%
% task:
% - fixation onset
% - stimulus onset (stimbit)
% - mask onset (microbit)
% - final cue onset 
% - fixation point green (targetbit)
%
%
%
close all
...
%124 - 1st session with variable stim dur, no mask. does not work with this
%script any more
%125-131  sessions with variable stim dur & mask
Sessions = 125:131;%125:128;
SNR = zeros(1,48);
...
%                     microbits = EVENT.strons.Micr;
%                     targetbits = EVENT.strons.Targ;
%                     mb = []; tb = [];
%                     for m = 1:length(microbits)
%                         mb(m) = microbits(m); 
%                         tb(m) = targetbits(find(targetbits>mb(m),1,'first'));
%                     end
...
%         hold on; plot(Poss_dur(1:min(numel(Poss_dur),6)), AUC_avg, 'ok'); plot(Poss_dur(1:min(numel(Poss_dur),6)), AUC_m_avg, 'og')
        ...
        % save AUC
        filename = ['Monty_varCue_AUC_s' num2str(Sessions(1)) '_' num2str(Sessions(end))];
