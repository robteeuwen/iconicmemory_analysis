% analysis function for the decay of iconic memory estimate 
% This script can be used for the initial decay estimation experiment
% (s22-26) 
...
for n = 1:48
        ...
        for bloc = 1:numel(info(s).goodblocs)
            ...
            if isempty(AllTrials)
                ...
                elseif s>=15 && s<20
                    ...
                    mask = 3*LOG.Masked(trials).*(LOG.Mask_delay(trials)<60)+ LOG.Masked(trials).*(LOG.Mask_delay(trials)>60);
                else
                ...
                Poss_dur = [Poss_dur unique(mask_delay)];
                mask_d = 2;
                if s<5
                ...
                elseif s>=15 && s<20
                    mask = [mask (3*LOG.Masked(trials).*(LOG.Mask_delay(trials)<60)+ LOG.Masked(trials).*(LOG.Mask_delay(trials)>60))];   
                else
    ...
    for h = 1:2
        if plot_stim_dur
            ...
            ind1 = find(connected==1  & mask ==1 & Hit==2 );%
            t_connected{n} =  nanmean(AllTrials(:,ind1),2);
            ind2 = find(connected==2  & mask ==1 & Hit==2 );%
...
%             figure; hist(nanmean(AllTrials(pf2,ind1)),20); hold on; hist(nanmean(AllTrials(pf2,ind2)),20);title(num2str(n));pause(0.1);
            ...
            if plot_mask
                for sd = 1:numel(Poss_dur)
                    ind = find(isdrawn4==0 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3 );%& stim_dur == Poss_dur(sd) & mask == m
                    background_m{n,h,sd}  = nanmean(AllTrials(:,ind),2);
                    
                    
                    ind = find(targ==10 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3);
                    ttree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    
                    
                    ind = find(targ==9 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3);
                    ttree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    ind = find(connected==1 & targ~=9 & Hit==h & mask_delay == Poss_dur(sd)& mask ==3 );
                    dtree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    
                    ind = find(connected==2 & targ~=10 & Hit==h & mask_delay == Poss_dur(sd)& mask ==3 );
                    dtree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    ind = find( Hit==h & mask_delay == Poss_dur(sd) & mask ==3);
                    perf_m(h,sd) = numel(ind);
                    
                    
                    ind = find(connected==1 & mask_delay == Poss_dur(sd) & mask ==3 & Hit==2);%
                    t_connected_m{n,sd} =  nanmean(AllTrials(:,ind),2);
                    ind = find(connected==2 & mask_delay == Poss_dur(sd) & mask ==3 & Hit==2);%
    ...
    for n = 1:48
        ...
        if plot_stim_dur
            ...
            for sd = 1:6
                ...
                if plot_mask
                    plot(tb, smooth(t_connected_m{n,sd}, smoothness), 'b', 'LineWidth',1); try  ylim([min(smooth(t_connected_m{n,sd},smoothness))-(max(smooth(t_connected_m{n,sd},smoothness))-min(smooth(t_connected_m{n,sd},smoothness))),  max(smooth(t_connected_m{n,sd},smoothness))+.2*(max(smooth(t_connected_m{n,sd},smoothness))-min(smooth(t_connected_m{n,sd},smoothness)))]);end
                    [hAx,hLine1,hLine2] = plotyy(tb, smooth(t_unconnected_m{n,sd}, smoothness), tb, smooth(t_connected_m{n,sd}-t_unconnected_m{n,sd}, smoothness));
                    diffcu =smooth(t_connected_m{n,sd}-t_unconnected_m{n,sd},smoothness);
                    hAx(2).XLim = [-.2 0.9];
                    hAx(1).XLim = [-.2 0.9];
                    hLine1.LineWidth = 1;
                    hLine1.Color = [1 0 0];
            ...
            subplot(2,4,sd+1);hold on; title('no mask')
            plot(tb, smooth(t_connected{n}, smoothness),'b');  try  ylim([min(smooth(t_connected{n},smoothness))-(max(smooth(t_connected{n},smoothness))-min(smooth(t_connected{n},smoothness))),  max(smooth(t_connected{n},smoothness))+.2*(max(smooth(t_connected{n},smoothness))-min(smooth(t_connected{n},smoothness)))]);end
            [hAx,hLine1,hLine2] = plotyy(tb, smooth(t_unconnected{n}, smoothness),tb, smooth(t_connected{n}-t_unconnected{n}, smoothness)); xlim([-.2 0.9]);
            hLine1.Color = [1 0 0];
            hLine2.Color=[0 0 0];
            hAx(1).XLim = [-.2 0.9];
            hAx(2).XLim = [-.2 0.9];
...
%             AUC = 0;
            ...
            diff = abs(smooth(t_connected{n},smoothness)- smooth(t_unconnected{n},smoothness));
...
%                 AUC_m = zeros(1,6);
                for sd = 1:6
                    ...
                    diff_m =abs(smooth(t_connected_m{n,sd}, smoothness)-smooth(t_unconnected_m{n,sd}, smoothness));
                    diff_m = diff_m(t_relev);diff_m(diff_m<0)=0;
%                     i = find(diff_m<0, 1);diff_m(i:end)=0;
                    AUC_m(sd,n) = sum(diff_m);
                end
                subplot(2,4,8);hold on; plot(400, AUC(n), 'bo'); plot(Poss_dur(1:6), AUC_m(:,n), 'r')
            end
...
if plot_stim_dur
    ...
    xlabel('Stimulus-Mask interval (ms)')
    if plot_mask
        hold on;  plot(Poss_dur(1:6),  perf_m(2,1:6)./sum(perf_m(:,1:6)),'r')
        legend('no mask', 'masked')
    ...
    for sd = 1:6
        ...
        figure(f1);title('no mask');hold on; %%subplot(2,4,sd)
    ...
    if plot_mask
        figure(f2)
        filename = 'NeurActiv_withStimDur_Mask';
        set(gcf, 'PaperPositionMode', 'auto')
        print(gcf, '-r0', [SaveDir filename, '.png'], '-dpng');
    end
    %%
    ...
    for sd = 1:6
        subplot(2,4,sd);hold on; title(['Mask delay:' num2str(Poss_dur(sd)), ' ms'])
        ylim1 = [-2 1.5];
        ...
        if plot_mask
            ...
            a = plot(tb, smooth(t_connected_m_avg{sd}, smoothness), 'b'); ylim(ylim1)
            [hAx,hLine1,hLine2] = plotyy(tb, smooth(t_unconnected_m_avg{sd}, smoothness), tb, smooth(t_connected_m_avg{sd}-t_unconnected_m_avg{sd}, smoothness));
            hAx(2).XLim = [-.2 0.9];
            hAx(1).XLim = [-.2 0.9];
%             hLine1.LineWidth = 1;
            hLine1.Color = [1 0 0];
...
%             hLine2.LineWidth = 1;
            axes(hAx(2));line(hAx(2).XLim,[0 0])
            if sd==1
               legend([a, hLine1,hLine2], { 'masked connected', 'mask unconnected', 'mask connected- unconn'})
            end
            %             plot(tb, smooth(t_connected_m_avg{sd}-t_unconnected_m_avg{sd}, smoothness), 'g', 'LineWidth',2);
        end
     ...
     subplot(2,4,sd+1);hold on; title('no mask')
     t_connected_avg = t_connected_avg-mean(t_connected_avg(tb<0));
     t_unconnected_avg = t_unconnected_avg-mean(t_unconnected_avg(tb<0));
    b = plot(tb, smooth(t_connected_avg, smoothness),'Color', [0 0 .7]);  ylim(ylim1);
        [hAx2,hLine12,hLine22] = plotyy(tb, smooth(t_unconnected_avg, smoothness),tb, smooth(t_connected_avg-t_unconnected_avg, smoothness)); xlim([-.2 0.9]);
        hLine12.Color = [.7 0 0];
        hLine22.Color=[0 0 0];
        hAx2(2).XLim = [-.2 0.9];
        hAx2(2).YLim = [-.6 2];
     ...
     p=patch([fix_stim_dur fix_stim_dur+integ_dur fix_stim_dur+integ_dur fix_stim_dur],[-10 -10 20 20],'r');
        set(p,'FaceAlpha',0.2, 'EdgeColor', 'none' );
        legend([b ,hLine12,hLine22], {'connected', 'unconnected', 'conn - unconn'})
        
         subplot(2,4,8);hold on;
%          AUC_avg = 0;
         t_relev = find(tb>(fix_stim_dur) & tb<(fix_stim_dur +integ_dur));
            diff = abs(smooth(t_connected_avg,smoothness)- smooth(t_unconnected_avg,smoothness));
            diff = diff(t_relev);diff(diff<0)=0;
%             i = find(diff<0, 1);diff(i:end)=0;
            AUC_avg = sum(diff);
...
%             AUC_m(sd) = sum(smooth(t_connected_m_avg{sd}(t_relev)-t_unconnected_m_avg{sd}(t_relev), smoothness));
        ...
        hold on; plot(Poss_dur(1:6), AUC_m_avg, 'g')
    end
%     legend({'no mask', 'masked'})
    xlabel('mask delay(ms)')
    title('Area Under the difference curve from stim end to stim end+300ms')
    e = std(AUC_m(:,good_chann),1,2)'/2;
    [hAx,hLine1,hLine2] = plotyy(Poss_dur(1:6), AUC_m_avg,[Poss_dur(1:6) 200],  [perf_m(2,1:6)./sum(perf_m(:,1:6)) perf(2)/sum(perf)]);
    hold on; errorbar(Poss_dur(1:6), AUC_m_avg,e, 'g')
    hLine2.Color = [1 0 0];
    hLine1.Color = [0 .8 0];
    hAx(1).YLim = [0 70];
    hAx(2).YLim = [.5 .8];
    legend([h1;hLine1;hLine2],'no mask','mask','behavior','Location','southeast');
    ...
    Mask_delay = [Poss_dur(1:6) NaN];
    behav_dacay = [perf_m(2,1:6)./sum(perf_m(:,1:6)) perf(2)/sum(perf)];
    filename = ['Monty_varMask_NeurBehav_s' num2str(Sessions(1)) '_' num2str(Sessions(end))];
    save(filename, 'neur_decay', 'behav_dacay', 'Mask_delay')
    %%
...
%     interval 50-400
    ...
    for t = 1:numel(a)
        ...
        cum_int(t) = sum(t_connected_avg(t_relev)-t_unconnected_avg(t_relev));
    end
...
%     value = .5*(sum(t_connected_avg(t_relev)-t_unconnected_avg(t_relev))+ sum(t_connected_avg{6}(t_relev)-t_unconnected_avg{6}(t_relev))) ;%- sum(t_connected_avg{3}(t_relev)-t_unconnected_avg{3}(t_relev));
    ...
    col = col(1:10:end,:);
     for sd = 1:6
         plot(tb, smooth(t_connected_m_avg{sd}-t_unconnected_m_avg{sd}, smoothness), 'Color', col(sd,:))
     end
    plot(tb, smooth(t_connected_avg-t_unconnected_avg, smoothness), 'k', 'LineWidth',2); xlim([-.2 0.9]);
