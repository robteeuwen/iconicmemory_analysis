...
% continuous segment in RF: 97-110
% disappearing segment in RF: 11-117
Sessions = 111:117;%
SNR = zeros(1,48);
...
%                         keyboard
            ...
            if isempty(AllTrials)
                AllTrials=e; % time x all trials (from all sessions)
    ...
    if s<115
        ...
        pf = find(tb > 0.05 & tb < 0.1); % tb is timeline - pf seems to be 50ms window after stimulus onset
        pf2 = find(tb > 0.05 & tb < 0.09);
    end

    hitNorm =(targ==curveInRF & correct==2);
    ...
    for h = 1:2
        for ct = 1:numel(Poss_Cue_Time)
            ...
            for ang = 1:numel(Poss_Angle)
                if plot_corr
                    ind_a{ang} = find(targ==curveInRF  & correct==h & cue_time==Poss_Cue_Time(ct) & angle==Poss_Angle(ang));%
                else
                    ind_a{ang} = find(targ==curveInRF  & cue_time==Poss_Cue_Time(ct) & angle==Poss_Angle(ang));
                end
            end
            ...
            for ang = 1:numel(Poss_Angle)
                if plot_corr
                    ind_a{ang} = find(targ==curveOutRF & correct==h  & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                else
                    ind_a{ang} = find(targ==curveOutRF  & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                end
            end
            mintrialspercond = min(cellfun(@numel,ind_a));
            ind = [];
            ...
            for ang = 1:numel(Poss_Angle)
                if plot_corr
                    ind_a{ang} = find(islong==1 & targ~=curveInRF  & correct==h & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                else
                    ind_a{ang} = find(islong==1  & targ~=curveInRF & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                end
            end
            mintrialspercond = min(cellfun(@numel,ind_a));
            ind = [];
            ...
            for ang = 1:numel(Poss_Angle)
                if plot_corr
                    ind_a{ang} = find(islong==0 & targ~=curveOutRF & correct==h & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                else
                    ind_a{ang} = find(islong==0 & targ~=curveOutRF & cue_time==Poss_Cue_Time(ct)& angle==Poss_Angle(ang));
                end
            end
            mintrialspercond = min(cellfun(@numel,ind_a));
            ind = [];
...
cdir = [SaveDir 's_' num2str(days(1)) '_' num2str(days(end)) '\'];
mkdir(cdir);
    ...
    for n = ana_chan
        ...
        for ct = 1:min(numel(Poss_Cue_Time),6)
            ...
            if Poss_Cue_Time(ct)<0
                p=patch([0.1+integ_start 0.1+integ_dur 0.1+integ_dur 0.1+integ_start],[-10 -10 20 20],'r');
                t_relev = find(tb>(0.1+integ_start) & tb<(0.1+integ_dur));
            else
                p=patch([fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_dur, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_dur, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start],[-10 -10 20 20],'r');
                t_relev = find(tb>(fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start) & tb<(fix_stim_dur+Poss_Cue_Time(ct)*1e-3 +integ_dur));
            end
        ...
        print(gcf,'-dpng',[cdir 'ConnUnconn_Chan' num2str(n) '_s' num2str(days(1)) '_' num2str(days(end)) '.png'])
...
set(gcf, 'PaperPositionMode', 'auto')
print(gcf, '-r0', [cdir filename, '.png'], '-dpng');
%% plot the average over an array for all the conditions
f2 = figure('Position', [394         234        1087         578]);
for ct = 1:min(numel(Poss_Cue_Time),6)
    % compute the average
    for h = 1:2
    ...
    % plot
    figure(f2);subplot(2,3,ct);hold on; 
    if ct==1
        title(['cued at ' num2str(-1*LOG.Precue_dur(end)+Poss_Cue_Time(ct)), ' ms']);
    else
        title(['cued at ' num2str(Poss_Cue_Time(ct)), ' ms'])%
    end
%     plot(tb,smooth(ttree_t_avg{1,ct},smoothness),'Color',[0 0 1]);
%     plot(tb,smooth(ttree_d_avg{1,ct},smoothness),'Color',[1 0 0]);
%     plot(tb,smooth(dtree_t_avg{1,ct},smoothness),'Color',[87/255 143/255 1]);
%     plot(tb,smooth(dtree_d_avg{1,ct},smoothness),'Color',[241/255 117/255 168/255]);
    
    if Poss_Cue_Time(ct)<0
        p=patch([0.1+integ_start 0.1+integ_dur .1+integ_dur 0.1+integ_start],[-10 -10 20 20],'r');
        t_relev = find(tb>(0.1+integ_start) & tb<(0.1+integ_dur));
    else
        p=patch([fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_dur, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_dur, fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start],[-10 -10 20 20],'r');
        t_relev = find(tb>(fix_stim_dur+Poss_Cue_Time(ct)*1e-3+integ_start) & tb<(fix_stim_dur+Poss_Cue_Time(ct)*1e-3 +integ_dur));
    ...
    q1 = plot(tb,smooth(ttree_t_avg{2,ct},smoothness),'Color',[0 0 1], 'LineWidth',1);
    q2 = plot(tb,smooth(ttree_d_avg{2,ct},smoothness),'Color',[1 0 0], 'LineWidth',1);
    q3 = plot(tb,smooth(dtree_t_avg{2,ct},smoothness),'Color',[87/255 143/255 1], 'LineWidth',1);
    q4 = plot(tb,smooth(dtree_d_avg{2,ct},smoothness),'Color',[241/255 117/255 168/255], 'LineWidth',1); xlim([-.4 0.9]);
    xlabel('time(s)');
    ...
    if Poss_Cue_Time(ct)>=0
        line([fix_stim_dur+1e-3*Poss_Cue_Time(ct) fix_stim_dur+1e-3*Poss_Cue_Time(ct)], [-10 20],'Color',[0 1 0],'LineWidth',1)
    else 
        cueonset = -1*LOG.Precue_dur(end)+Poss_Cue_Time(ct);
        l1 = line([cueonset/1000 cueonset/1000], [-10 20],'Color',[0 1 0],'LineWidth',1);
    end
    l2 = line([650/1000 650/1000],[-10 20],'Color',[0 0 0],'LineWidth',1);
    if ct == 5
        L = legend([q1 q2 q3 q4 l1 l2],{'target', 'target unconnected', 'distractor', 'distractor unconnected', 'cue onset', 'response cue'});
        set(L,'Position',[0.6359 0.2725 0.1592 0.1773]);
    end
    ylim([-.2 1])
end
...
f3 = figure('Position', [100 50 1400 900]);
figure(f3);clf
...
smoothness = 50;
for ct = 1:min(numel(Poss_Cue_Time),6)
    topplot = ct+4*floor((ct-1)/4);
    bottomplot = ct+4*floor((ct-1)/4)+4;
    
    subplot(4,4,topplot);hold on; title(['cue delay:' num2str(Poss_Cue_Time(ct)), ' ms'])
    ylim1 = [-0.2 0.8];
    xlabel('time(s)'); ylabel('normalized activity')
    ttree_t_avg{2,ct} = ttree_t_avg{2,ct}-mean(ttree_t_avg{2,ct}(tb<baseline(2)));
    ttree_d_avg{2,ct} = ttree_d_avg{2,ct}-mean(ttree_d_avg{2,ct}(tb<baseline(2)));
    dtree_t_avg{2,ct} = dtree_t_avg{2,ct}-mean(dtree_t_avg{2,ct}(tb<baseline(2)));
    dtree_d_avg{2,ct} = dtree_d_avg{2,ct}-mean(dtree_d_avg{2,ct}(tb<baseline(2)));
    a = plot(tb, smooth(ttree_t_avg{2,ct}, smoothness)-smooth(ttree_d_avg{2,ct}, smoothness), 'b'); 
    b = plot(tb, smooth(dtree_t_avg{2,ct}, smoothness)-smooth(dtree_d_avg{2,ct}, smoothness), 'r');
    ...
    c = plot(tb, smooth(ttree_t_avg{2,ct}-ttree_d_avg{2,ct}-dtree_t_avg{2,ct}+dtree_d_avg{2,ct}, smoothness),'k');
    xlabel('time(s)');
    ylabel('normalized activity');
...
%     axes(hAx(2));line(hAx(2).XLim,[0 0])
    if ct==5
        L = legend([a, b,c], { 'target pair', 'distractor pair', 'difference'});
        set(L,'Position',[0.3019    0.4152    0.0886    0.0589]);
    end
...
%% special integration
integ_dur = 0.35; % response onset
integ_total = (integ_dur-integ_start)*1000;
t_relev = find(tb>(fix_stim_dur+Poss_Cue_Time(2)*1e-3+integ_start) & tb<(fix_stim_dur+Poss_Cue_Time(2)*1e-3 +integ_dur));
t_relev_pre = find(tb>0.1 & tb<(0.1+integ_dur));

% difference between curves with cue at 0ms 
ct = 2;
diff =(smooth(ttree_t_avg{2,ct}-ttree_d_avg{2,ct}, smoothness)-smooth(dtree_t_avg{2,ct}-dtree_d_avg{2,ct}, smoothness));
d1 = diff(t_relev);

% difference between curves for precue
ct = 1;
diff =(smooth(ttree_t_avg{2,ct}-ttree_d_avg{2,ct}, smoothness)-smooth(dtree_t_avg{2,ct}-dtree_d_avg{2,ct}, smoothness));
d2 = diff(t_relev_pre);

% difference between curves for latest cue
ct = 5;
diff =(smooth(ttree_t_avg{2,ct}-ttree_d_avg{2,ct}, smoothness)-smooth(dtree_t_avg{2,ct}-dtree_d_avg{2,ct}, smoothness));
d3 = diff(t_relev);

neuronal_worth = (sum(d1)/sum(d2))*stim_dur(end);
neuronal_worth = ((sum(d1)-sum(d3))/(sum(d2)-sum(d3)))*stim_dur(end);
