...
xtractdir = 'D:\Projects\IconicMemory\datasets\Darwin\extractdata\20181002\';
mkdir(xtractdir)
...
%         e2chn = EVENT.strms(find(fld2)).channels;
        ...
        if diodeon
            % get photodiode
            EVENT.Myevent = 'ANA1';
            EVENT = rmfield(EVENT,'CHAN');
            Dio = Exd4(EVENT,Trials);
            Dio = Dio{1};
            save([xtractdir,savediodename,'_Block-',num2str(Blockno(B))],'Dio')
        end
    end
end
