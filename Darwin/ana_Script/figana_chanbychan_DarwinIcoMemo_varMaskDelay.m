...
load([extractdir, sessions{36} '\EVT_', info(36).Tankname,'_block-',num2str(info(36).goodblocs(1))])
...
for n = 25:48
    ...
    AllCond = [];
    days =[]; 
    for s = Sessions%49:59;%49:50%%32:33%31 
...
%      Fix_sd = 60;
    for h = 1:2
        if plot_stim_dur
            ...
            ind = find(isdrawn4==0 & Hit==h  & mask ==1 & stim_dur>100);%& stim_dur == Poss_dur(sd) & mask == m
...
%             figure; hist(nanmean(AllTrials(pf2,ind1)),20); hold on; hist(nanmean(AllTrials(pf2,ind2)),20);title(num2str(n));pause(0.1);
            ...
            if plot_mask
                for sd = 1:numel(Poss_dur)
                    ind = find(isdrawn4==0 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3 & stim_dur>100);%& stim_dur == Poss_dur(sd) & mask == m
                    background_m{n,h,sd}  = nanmean(AllTrials(:,ind),2);
                    
                    
                    ind = find(targ==5 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3 & stim_dur>100);
                    ttree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    
                    
                    ind = find(targ==6 & Hit==h & mask_delay == Poss_dur(sd) & mask ==3 & stim_dur>100);
                    ttree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    ind = find(connected==1 & targ~=5 & Hit==h & mask_delay == Poss_dur(sd)& mask ==3  & stim_dur>100);
                    dtree_t_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    
                    ind = find(connected==2 & targ~=6 & Hit==h & mask_delay == Poss_dur(sd)& mask ==3 & stim_dur>100 );
                    dtree_d_m{n,h,sd} = nanmean(AllTrials(:,ind),2);
                    
                    ind = find( Hit==h & mask_delay == Poss_dur(sd) & mask ==3 & stim_dur>100);
                    perf_m(h,sd) = numel(ind);
                    
                    
                    ind = find(connected==1 & mask_delay == Poss_dur(sd) & mask ==3 & Hit==2 & stim_dur>100);%
                    t_connected_m{n,sd} =  nanmean(AllTrials(:,ind),2);
                    ind = find(connected==2 & mask_delay == Poss_dur(sd) & mask ==3 & Hit==2 & stim_dur>100);%
            ...
            if plot_mask
                ind = find(isdrawn4==0 & Hit==h & mask ==3  & stim_dur>100);%& stim_dur == Poss_dur(sd) & mask == m
...
%             AUC = 0;
            ...
            diff = smooth(t_connected{n},smoothness)- smooth(t_unconnected{n},smoothness);
...
%                 AUC_m = zeros(1,6);
                for sd = 1:min(numel(Poss_dur),6)
                    ...
                    diff_m =smooth(t_connected_m{n,sd}, smoothness)-smooth(t_unconnected_m{n,sd}, smoothness);
...
%          AUC_avg = 0;
         t_relev = find(tb>(fix_stim_dur) & tb<(fix_stim_dur +integ_dur));
            diff = smooth(t_connected_avg,smoothness)- smooth(t_unconnected_avg,smoothness);
            diff = diff(t_relev);diff(diff<0)=0;
%             i = find(diff<0, 1);diff(i:end)=0;
            AUC_avg = sum(diff);
...
%     legend({'no mask', 'masked'})
    ...
    Mask_delay = [Poss_dur(1:min(numel(Poss_dur),6)) NaN];
    behav_dacay = [perf_m(2,1:min(numel(Poss_dur),6))./sum(perf_m(:,1:min(numel(Poss_dur),6))) perf(2)/sum(perf)];
    filename = ['Darwin_varMask_NeurBehav_s' num2str(Sessions(1)) '_' num2str(Sessions(end))];
